...
public final class AudioStream implements AutoCloseable {
    static final int EXPECTED_NATIVE_VERSION_CODE = 3;
    ...
    static void ensureNativeVersion() {
        int actualVersionCode = nativeVersionCode();
        if (actualVersionCode != EXPECTED_NATIVE_VERSION_CODE) {
            throw new IllegalStateException(
                    "native library version mismatch: expected "
                            + EXPECTED_NATIVE_VERSION_CODE
                            + " but was "
                            + actualVersionCode
                            + ". Rebuild liboboe_jni.so with tools/build-rust-android.");
        }
    }
    ...
    static native int nativeGetLastOpenError();
    ...
    private static native int nativeGetSessionId(long handle);
