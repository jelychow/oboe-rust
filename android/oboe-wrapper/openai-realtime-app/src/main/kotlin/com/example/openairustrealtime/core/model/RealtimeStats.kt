...
data class RealtimeStats(
    ...
    companion object {
        private val statsPattern: Pattern = Pattern.compile(
            "Mic sent: (\\d+) chunks / (\\d+) frames\\. Output played: (\\d+) chunks / (\\d+) frames\\.(?: Levels: mic ([0-9.]+), output ([0-9.]+)\\.)?"
        )
        ...
        fun parse(text: String?): RealtimeStats {
            ...
            return RealtimeStats(
                ...
                outputChunks = matcher.group(3).toSafeLong(),
                outputFrames = matcher.group(4).toSafeLong(),
                micLevel = matcher.group(5).toSafeFloat(),
                outputLevel = matcher.group(6).toSafeFloat()
            )
        }
