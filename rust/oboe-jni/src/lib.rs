...
use oboe_core::types::{
    AudioApi, ContentType, Direction, Format, InputPreset, PerformanceMode, SharingMode, Usage,
};
...
const OPEN_ERROR_NONE: jint = 0;
const OPEN_ERROR_INVALID_ARGUMENT: jint = -2;
const OPEN_ERROR_INVALID_STATE: jint = -3;
const OPEN_ERROR_CLOSED: jint = -4;
const OPEN_ERROR_UNAVAILABLE: jint = -5;
const OPEN_ERROR_BACKEND_UNAVAILABLE: jint = -6;
const OPEN_ERROR_INTERNAL: jint = -7;
const OPEN_ERROR_UNIMPLEMENTED: jint = -8;

thread_local! {
    static LAST_OPEN_ERROR: Cell<jint> = const { Cell::new(OPEN_ERROR_NONE) };
}
...
impl NativeStream {
    fn open(builder: StreamBuilder) -> oboe_core::error::Result<Self> {
        match builder.api {
            AudioApi::AAudio => AAudioBackend::open(&builder).map(Self::AAudio),
            AudioApi::Unspecified => match AAudioBackend::open(&builder).map(Self::AAudio) {
                Ok(stream) => Ok(stream),
                Err(aaudio_error) => {
                    let mut fallback = builder.clone();
                    fallback.api = AudioApi::OpenSLES;
                    OpenSLESBackend::open(&fallback)
                        .map(Self::OpenSLES)
                        .map_err(|_| aaudio_error)
                }
            },
    ...
    fn get_session_id(&self) -> oboe_core::error::Result<i32> {
        match self {
            Self::AAudio(stream) => stream.get_session_id(),
            Self::OpenSLES(stream) => stream.get_session_id(),
        }
    }
...
fn set_last_open_error(error: jint) {
    LAST_OPEN_ERROR.with(|last_error| last_error.set(error));
}

fn get_last_open_error() -> jint {
    LAST_OPEN_ERROR.with(Cell::get)
}

fn open_error_code(error: Error) -> jint {
    match error {
        Error::InvalidArgument => OPEN_ERROR_INVALID_ARGUMENT,
        Error::InvalidState => OPEN_ERROR_INVALID_STATE,
        Error::Closed => OPEN_ERROR_CLOSED,
        Error::Unavailable => OPEN_ERROR_UNAVAILABLE,
        Error::BackendUnavailable => OPEN_ERROR_BACKEND_UNAVAILABLE,
        Error::Internal => OPEN_ERROR_INTERNAL,
        Error::Unimplemented => OPEN_ERROR_UNIMPLEMENTED,
        Error::Platform(result) => result,
    }
}
...
fn input_preset_from_jint(input_preset: jint) -> Option<InputPreset> {
    match input_preset {
        0 => Some(InputPreset::Unspecified),
        1 => Some(InputPreset::Generic),
        5 => Some(InputPreset::Camcorder),
        6 => Some(InputPreset::VoiceRecognition),
        7 => Some(InputPreset::VoiceCommunication),
        9 => Some(InputPreset::Unprocessed),
        10 => Some(InputPreset::VoicePerformance),
        _ => None,
    }
}

fn usage_from_jint(usage: jint) -> Option<Usage> {
    match usage {
        0 => Some(Usage::Unspecified),
        1 => Some(Usage::Media),
        2 => Some(Usage::VoiceCommunication),
        3 => Some(Usage::VoiceCommunicationSignalling),
        4 => Some(Usage::Alarm),
        5 => Some(Usage::Notification),
        6 => Some(Usage::NotificationRingtone),
        10 => Some(Usage::NotificationEvent),
        11 => Some(Usage::AssistanceAccessibility),
        12 => Some(Usage::AssistanceNavigationGuidance),
        13 => Some(Usage::AssistanceSonification),
        14 => Some(Usage::Game),
        16 => Some(Usage::Assistant),
        _ => None,
    }
}

fn content_type_from_jint(content_type: jint) -> Option<ContentType> {
    match content_type {
        0 => Some(ContentType::Unspecified),
        1 => Some(ContentType::Speech),
        2 => Some(ContentType::Music),
        3 => Some(ContentType::Movie),
        4 => Some(ContentType::Sonification),
        _ => None,
    }
}
...
pub extern "system" fn Java_com_google_oboe_AudioStream_nativeOpen(
    ...
    set_last_open_error(OPEN_ERROR_NONE);
    ...
    let builder = StreamBuilder {
        ...
        direction: match direction_from_jint(direction) {
            ...
            None => {
                set_last_open_error(OPEN_ERROR_INVALID_ARGUMENT);
                return 0;
            }
        },
        ...
        input_preset: match input_preset_from_jint(input_preset) {
            Some(input_preset) => input_preset,
            None => {
                set_last_open_error(OPEN_ERROR_INVALID_ARGUMENT);
                return 0;
            }
        },
        usage: match usage_from_jint(usage) {
            Some(usage) => usage,
            None => {
                set_last_open_error(OPEN_ERROR_INVALID_ARGUMENT);
                return 0;
        ...
        content_type: match content_type_from_jint(content_type) {
    ...
    match NativeStream::open(builder) {
        Ok(stream) => lock_registry().insert(stream),
        Err(error) => {
            set_last_open_error(open_error_code(error));
            0
        }
    }
}
...
#[no_mangle]
pub extern "system" fn Java_com_google_oboe_AudioStream_nativeGetLastOpenError(
    _env: JNIEnv,
    _class: jclass,
) -> jint {
    get_last_open_error()
...
#[no_mangle]
pub extern "system" fn Java_com_google_oboe_AudioStream_nativeGetSessionId(
    _env: JNIEnv,
    _self: jobject,
    handle: jlong,
) -> jint {
    with_stream_i32(handle, NativeStream::get_session_id)
}
...
mod tests {
    ...
    fn invalid_handle_returns_error_codes() {
        ...
        assert_eq!(
            Java_com_google_oboe_AudioStream_nativeGetSessionId(null_mut(), null_mut(), 0),
            -1
        );
        assert_eq!(
    ...
    #[test]
    fn unknown_api_maps_to_unspecified() {
        assert_eq!(api_from_jint(99), AudioApi::Unspecified);
    }

    #[test]
    fn input_preset_mapping_accepts_voice_communication() {
        assert_eq!(
            input_preset_from_jint(7),
            Some(InputPreset::VoiceCommunication)
        );
        assert_eq!(input_preset_from_jint(99), None);
    }

    #[test]
    fn output_attribute_mappings_accept_voice_communication_speech() {
        assert_eq!(usage_from_jint(2), Some(Usage::VoiceCommunication));
        assert_eq!(content_type_from_jint(1), Some(ContentType::Speech));
        assert_eq!(usage_from_jint(99), None);
        assert_eq!(content_type_from_jint(99), None);
    }
    ...
    #[test]
    fn opensles_input_is_rejected_until_recorder_backend_exists() {
        assert_eq!(native_open_with_direction(2, 0), 0);
    }

    #[test]
    fn aaudio_handle_rejects_callback_config_mutation_but_accepts_extension_paths() {
        let handle = native_open(1);
        assert_ne!(handle, 0);
    ...
    #[test]
    fn opensles_handle_rejects_callback_path_and_aaudio_only_extensions() {
        let handle = native_open(2);
        assert_ne!(handle, 0);
    ...
    fn native_open_rejects_invalid_builder_config() {
        ...
        assert_eq!(handle, 0);
        assert_eq!(
            Java_com_google_oboe_AudioStream_nativeGetLastOpenError(null_mut(), null_mut()),
            OPEN_ERROR_INVALID_ARGUMENT
        );
    }

    #[test]
    fn successful_native_open_clears_last_open_error() {
        let failed = Java_com_google_oboe_AudioStream_nativeOpen(
            null_mut(),
            null_mut(),
            1,
            1,
