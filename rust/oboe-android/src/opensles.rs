...
use oboe_core::types::{Direction, Format};
...
mod platform {
    ...
    type SLAndroidSimpleBufferQueueCallback =
        unsafe extern "C" fn(SLAndroidSimpleBufferQueueItf, *mut c_void);
    ...
    const OPENSL_QUEUE_DEPTH: usize = 2;
    ...
    struct SLAndroidSimpleBufferQueueItf_ {
        ...
        RegisterCallback: Option<
            unsafe extern "C" fn(
                SLAndroidSimpleBufferQueueItf,
                Option<SLAndroidSimpleBufferQueueCallback>,
                *mut c_void,
            ) -> SLresult,
        >,
    }
    ...
    struct OpenSLESQueueState {
        completed_buffers: AtomicUsize,
    }
    ...
    pub(super) struct OpenSLESPlatformStream {
        ...
        pending_i16: VecDeque<Vec<i16>>,
        queue_state: Box<OpenSLESQueueState>,
    }
    ...
    impl OpenSLESPlatformStream {
        pub(super) fn open(builder: &StreamBuilder) -> Result<Self> {
            ...
            let queue_state = Box::new(OpenSLESQueueState {
                completed_buffers: AtomicUsize::new(0),
            });
            let queue_context =
                (&*queue_state as *const OpenSLESQueueState).cast::<c_void>() as *mut c_void;
            if let Err(error) =
                register_queue_callback(queue, Some(buffer_queue_callback), queue_context)
            {
                destroy_object(&mut player_object);
                destroy_object(&mut output_mix_object);
                destroy_object(&mut engine_object);
                return Err(error);
            }
        ...
        pub(super) fn request_stop(&mut self) -> Result<()> {
            ...
            let result = clear_queue(self.queue);
            self.pending_i16.clear();
            self.queue_state
                .completed_buffers
        ...
        pub(super) fn write_f32(
            ...
            if !self.wait_for_queue_slot(timeout_nanos)? {
                return Ok(0);
            }

            let buffer = audio.iter().copied().map(float_to_i16).collect::<Vec<_>>();
            let byte_count = buffer
                .len()
                .checked_mul(core::mem::size_of::<i16>())
                .and_then(|bytes| SLuint32::try_from(bytes).ok())
                .ok_or(Error::InvalidArgument)?;
            enqueue(self.queue, buffer.as_ptr().cast(), byte_count)?;
            self.pending_i16.push_back(buffer);
            Ok(audio.len() as i32)
        }
        ...
        fn destroy_player(&mut self) {
            if !self.queue.is_null() {
                let _ = register_queue_callback(self.queue, None, ptr::null_mut());
                let _ = clear_queue(self.queue);
            }
        ...
        fn wait_for_queue_slot(&mut self, timeout_nanos: i64) -> Result<bool> {
            self.reap_completed_buffers();
            if self.pending_i16.len() < OPENSL_QUEUE_DEPTH {
                return Ok(true);
            }
            if timeout_nanos < 0 {
                return Err(Error::InvalidArgument);
            ...
            let deadline = Instant::now()
                .checked_add(Duration::from_nanos(timeout_nanos as u64))
                .ok_or(Error::InvalidArgument)?;
            while self.pending_i16.len() >= OPENSL_QUEUE_DEPTH {
                self.reap_completed_buffers();
                if self.pending_i16.len() < OPENSL_QUEUE_DEPTH {
                    return Ok(true);
                }
        ...
        fn reap_completed_buffers(&mut self) {
            let completed = self
                ...
                .min(self.pending_i16.len());
            for _ in 0..completed {
                self.pending_i16.pop_front();
            }
        }
    }
    ...
    fn register_queue_callback(
        queue: SLAndroidSimpleBufferQueueItf,
        callback: Option<SLAndroidSimpleBufferQueueCallback>,
        context: *mut c_void,
    ) -> Result<()> {
        let register = queue_vtable(queue)?
            .RegisterCallback
            .ok_or(Error::InvalidState)?;
        sl_result(unsafe { register(queue, callback, context) })
    }

    unsafe extern "C" fn buffer_queue_callback(
        _queue: SLAndroidSimpleBufferQueueItf,
        context: *mut c_void,
    ) {
        if context.is_null() {
            return;
        }
        let state = unsafe { &*context.cast::<OpenSLESQueueState>() };
        state.completed_buffers.fetch_add(1, Ordering::AcqRel);
    }
...
mod tests {
    ...
    #[test]
    fn opensles_backend_rejects_input_until_recorder_exists() {
        let builder = StreamBuilder {
            direction: oboe_core::types::Direction::Input,
            ..StreamBuilder::default()
        };
