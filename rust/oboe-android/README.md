...
Supported in `0.1.0-alpha.6`:

- AAudio stream open/start/stop/close.
- AAudio blocking `f32` read/write.
- AAudio `f32` and `i16` conversion paths.
- Rust-native AAudio data/error callback binding for `f32` streams.
- AAudio timestamp, frame counter, xrun, burst, and buffer size tuning queries.
- OpenSL ES blocking output enqueue for `f32`/`i16` conversion.

Known alpha limitations:

- OpenSL ES input is not a real recorder path.
- Java realtime callback dispatch is not wired to native audio callback threads.
- Adaptive latency tuning, pause, flush, and release APIs are not yet exposed.
