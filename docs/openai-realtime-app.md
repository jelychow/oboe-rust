...
- Android UI: session API key, TTS, ASR, realtime chat, realtime translate,
  status badge, realtime signal visualization, audio counters,
  transcript/result panel, event feed, and clear-key controls

The API key is entered at runtime and can be saved on-device in encrypted form.
The sample stores the key in `SharedPreferences`, encrypted with an AES key from
Android Keystore. The UI does not refill the full saved key back into the input
field; leave the field blank to reuse the saved key. Use a backend-issued
ephemeral credential before shipping this app to end users.
...
1. Enter an OpenAI API key and tap Save Key to store it on the device.
   The field stays blank after save; leave it blank later to reuse the saved key,
   or enter a new one to replace it.
...
6. Use Realtime Translate:
   ...
   - Speak to the device; translated assistant audio and transcript/status
     updates appear live.
7. Tap Clear to remove the saved on-device key.
...
- `core.data`: session API key handoff and repository orchestration.
...
- The sample encrypts and persists the API key on-device for local development
  convenience. It still passes the decrypted key directly to network calls, so
  do not distribute this as a production secret handling pattern.
- Temporary cached audio files are deleted after playback/transcription finishes
  or is cancelled.
