...
dependencies {
    implementation("io.github.jelychow.oboe:oboe-rust-android:0.1.0-alpha.4")
}
```
...
./gradlew :oboe-wrapper:publishReleasePublicationToMavenLocal \
  -PoboeRust.version=0.1.0-alpha.4
```
...
仓库内置 `.github/workflows/publish-github-packages.yml`，会在 GitHub Release 发布时自动验证并发布 Android wrapper package。建议 release tag 使用 `v0.1.0-alpha.4` 这种格式；workflow 会去掉前缀 `v`，最终发布 package version `0.1.0-alpha.4`。手动触发 workflow 仍然可用，适合显式重试某个版本。
