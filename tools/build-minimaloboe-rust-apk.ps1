...
$appJavaDir = Join-Path $repoRoot "android/oboe-wrapper/minimaloboe-rust-app/src/main/java"
$appManifest = Join-Path $repoRoot "android/oboe-wrapper/minimaloboe-rust-app/src/main/AndroidManifest.xml"
$buildRoot = Join-Path $repoRoot "build/minimaloboe-rust-apk"
$classesDir = Join-Path $buildRoot "classes"
$dexDir = Join-Path $buildRoot "dex"
$packagingDir = Join-Path $buildRoot "package"
$unsignedApk = Join-Path $buildRoot "minimaloboe-rust-unsigned.apk"
$alignedApk = Join-Path $buildRoot "minimaloboe-rust-aligned.apk"
...
if ([string]::IsNullOrWhiteSpace($OutputApk)) {
    $OutputApk = Join-Path $buildRoot "minimaloboe-rust-debug.apk"
}
...
$manifestText = $manifestText -replace '<manifest xmlns:android="http://schemas.android.com/apk/res/android">', '<manifest xmlns:android="http://schemas.android.com/apk/res/android" package="com.example.minimaloboe">'
...
Invoke-ExternalCommand `
    ...
    "javac failed for minimaloboe Rust APK sources."
...
Invoke-ExternalCommand `
    ...
    "d8 failed for minimaloboe Rust APK classes."
...
Invoke-ExternalCommand `
    ...
    "aapt2 link failed for minimaloboe Rust APK."

Copy-Item -LiteralPath (Join-Path $dexDir "classes.dex") -Destination (Join-Path $packagingDir "classes.dex")

$jniRoot = Join-Path $repoRoot "android/oboe-wrapper/minimaloboe-rust-app/src/main/jniLibs"
foreach ($library in Get-ChildItem -LiteralPath $jniRoot -Filter "libminimaloboe_rust.so" -Recurse) {
    ...
    Copy-Item -LiteralPath $library.FullName -Destination (Join-Path $abiOutput "libminimaloboe_rust.so") -Force
}
...
Invoke-ExternalCommand `
    ...
    "zipalign failed for minimaloboe Rust APK."
...
Invoke-ExternalCommand `
    ...
    "apksigner failed for minimaloboe Rust APK."

Invoke-ExternalCommand `
    $apksigner `
    @("verify", "--verbose", $OutputApk) `
    "apksigner verify failed for minimaloboe Rust APK."

Write-Output "MinimalOboe Rust APK built: $OutputApk"
