...
$appJavaDir = Join-Path $repoRoot "android/oboe-wrapper/oboe-samples-app/src/main/java"
$appManifest = Join-Path $repoRoot "android/oboe-wrapper/oboe-samples-app/src/main/AndroidManifest.xml"
$buildRoot = Join-Path $repoRoot "build/oboe-samples-apk"
$classesDir = Join-Path $buildRoot "classes"
$dexDir = Join-Path $buildRoot "dex"
$packagingDir = Join-Path $buildRoot "package"
$unsignedApk = Join-Path $buildRoot "oboe-samples-unsigned.apk"
$alignedApk = Join-Path $buildRoot "oboe-samples-aligned.apk"
...
if ([string]::IsNullOrWhiteSpace($OutputApk)) {
    $OutputApk = Join-Path $buildRoot "oboe-samples-debug.apk"
}
...
$manifestText = $manifestText -replace '<manifest xmlns:android="http://schemas.android.com/apk/res/android">', '<manifest xmlns:android="http://schemas.android.com/apk/res/android" package="com.google.oboe.samples">'
...
Invoke-ExternalCommand `
    ...
    "javac failed for samples APK sources."
...
Invoke-ExternalCommand `
    ...
    "d8 failed for samples APK classes."
...
Invoke-ExternalCommand `
    ...
    "aapt2 link failed for samples APK."

Copy-Item -LiteralPath (Join-Path $dexDir "classes.dex") -Destination (Join-Path $packagingDir "classes.dex")

$jniRoot = Join-Path $repoRoot "android/oboe-wrapper/oboe-samples-app/src/main/jniLibs"
foreach ($library in Get-ChildItem -LiteralPath $jniRoot -Filter "liboboe_samples_jni.so" -Recurse) {
    ...
    Copy-Item -LiteralPath $library.FullName -Destination (Join-Path $abiOutput "liboboe_samples_jni.so") -Force
}
...
Invoke-ExternalCommand `
    ...
    "zipalign failed for samples APK."
...
Invoke-ExternalCommand `
    ...
    "apksigner failed for samples APK."

Invoke-ExternalCommand `
    $apksigner `
    @("verify", "--verbose", $OutputApk) `
    "apksigner verify failed for samples APK."

Write-Output "Samples APK built: $OutputApk"
