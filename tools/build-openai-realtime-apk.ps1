param(
    ...
    [string]$NdkVersion = "",
    [string]$OutputApk = "",
    [string]$OboeDependency = "",
    [string]$OboeRemoteVersion = "",
...
if ($hasKotlinSources) {
    ...
    $gradleArguments = @("--no-daemon", ":openai-realtime-app:assembleDebug", "--console=plain")
    if (-not [string]::IsNullOrWhiteSpace($OboeDependency)) {
        $gradleArguments += "-PoboeRust.realtimeDependency=$OboeDependency"
    }
    if (-not [string]::IsNullOrWhiteSpace($OboeRemoteVersion)) {
        $gradleArguments += "-PoboeRust.remoteVersion=$OboeRemoteVersion"
    }
